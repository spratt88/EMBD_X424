...
has_arduino_core() {
    $ARDUINO_CLI core list | grep "arduino:mbed_nicla" || true
}
HAS_ARDUINO_CORE="$(has_arduino_core)"
if [ -z "$HAS_ARDUINO_CORE" ]; then
    echo "Installing Arduino Mbed core..."
    $ARDUINO_CLI core update-index
    $ARDUINO_CLI core install arduino:mbed_nicla@3.2.0
...
has_serial_port() {
    ($ARDUINO_CLI board list | grep "Arduino Nicla Vision" || true) | cut -d ' ' -f1
}
...
cd "$SCRIPTPATH"
$ARDUINO_CLI upload -p $SERIAL_PORT --fqbn $BOARD --input-dir $SCRIPTPATH
